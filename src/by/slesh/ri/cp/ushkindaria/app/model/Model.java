...
public class Model {
    ...
    public BufferedImage cut(BufferedImage source, int x1, int y1, int x2,
	    int y2) {

	BufferedImage target = new BufferedImage(x2 - x1, y2 - y1,
	        AbstractTool.IMAGE_TYPE);
    ...
    public BufferedImage trim(BufferedImage source, int indentTop,
	...
	for (int y = 0; y < h; ++y)
	    for (int x = x0; x < x0 + G.WIDTH; ++x)
		if (pixels[w * y + x] == AbstractTool._1)
		    if (y + G.HEIGHT <= h) neuralRecognize(x0, y);
	...
	BufferedImage target = new BufferedImage(source.getWidth(null),
	        source.getHeight(null), AbstractTool.IMAGE_TYPE);
     ...
     * O
     */
    int[]    yCoords    = new int[G.QUANTITY_NEURONS];
    double[] currentOut = new double[G.QUANTITY_NEURONS];

    public void neuralRecognize(int x, int y) {
	BufferedImage bi = mTargetImage.getSubimage(x, y, G.WIDTH, G.HEIGHT);
	int[] input = Helpers.initX(bi);
	int[] output = Perceptron.getInstance().recognize(input);
	for(int k = 0; k < output.length; ++k){
	    if(output[k] > currentOut[k]){
		currentOut[k] = output[k];
		yCoords[k] = y;
	    }
	}
    }
